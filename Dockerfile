...
ARG TENSORRT=TensorRT-8.0.3.4.Linux.x86_64-gnu.cuda-11.3.cudnn8.2.tar.gz

# 11.3.0-cudnn8-runtime-ubuntu20.04
# nvidia/cuda:11.3.0-cudnn8-devel-ubuntu20.04
# tensorrt-ubuntu18.04-cuda11.3
...
ARG TENSORRT
COPY ${TENSORRT} /app/
ENV LD_LIBRARY_PATH=$LD_LIBRARY_PATH:/app/TensorRT-8.0.3.4/lib
RUN tar -xvzf /app/TensorRT-8.0.3.4.Linux.x86_64-gnu.cuda-11.3.cudnn8.2.tar.gz -C /app/ \
    && rm /app/TensorRT-8.0.3.4.Linux.x86_64-gnu.cuda-11.3.cudnn8.2.tar.gz \
    && export TRT_LIBPATH=/app/TensorRT-8.0.3.4/lib \
...
RUN cd /app/onnx-tensorrt \
	...
	&& cmake .. -DTENSORRT_ROOT=/app/TensorRT-8.0.3.4 \
    && make -j$(nproc) \
...
# [Spconv]
# TODO: replace with new: pip install spconv-cu113
...
RUN /bin/bash -c "cd /app/PointPillars/ && \
    mkdir build && \
    cd build && \
    cmake .. -DCMAKE_BUILD_TYPE=Debug -DTENSORRT_ROOT=/app/TensorRT-8.0.3.4 && \
    make -j$(nproc)"
...
RUN /bin/bash -c "source /opt/ros/${ROS_DISTRO}/setup.sh && \
    ...
    catkin clean -yb && \
    catkin build --cmake-args -DCMAKE_BUILD_TYPE=Debug -DTENSORRT_ROOT=/app/TensorRT-8.0.3.4"
...
RUN /bin/bash -c "cd /app/PointPillars/ && \
    mkdir build && \
    cd build && \
    cmake .. -DCMAKE_BUILD_TYPE=Release -DTENSORRT_ROOT=/app/TensorRT-8.0.3.4 && \
    make -j$(nproc) && \
    make install"
...
RUN /bin/bash -c "source /opt/ros/${ROS_DISTRO}/setup.sh && \
    ...
    catkin config --init --install && \
    catkin clean -yb && \
    catkin build --cmake-args -DCMAKE_BUILD_TYPE=Release -DTENSORRT_ROOT=/app/TensorRT-8.0.3.4"
